...
export const testTicket = createAsyncThunk(
  "ticket/testTicket",
  async (testTicket, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        `${API_BASE_URL}tickets/testTicket`,
        testTicket
      );
      return response.data;
    } catch (error) {
...
const ticketSlice = createSlice({
  ...
  reducers: {
    setTicketsToBook: (state, action) => {
      state.ticketsToBook = action.payload;
    },
  },
...
export const { setTicketsToBook } = ticketSlice.actions;
